...
3. Mocha processes any command-line options provided
...
6. Mocha _discovers_ test files; when given no files or directories, it finds files with extensions `.js`, `.mjs` or `.cjs` in the `test` directory (but not its children), relative to the current working directory
7. The (default) [bdd interface](#bdd) loads the test files _in no particular order_, which are given an interface-specific `global` context (this is how, e.g., `describe()` ends up as a global in a test file)
   1. When a test file is loaded, Mocha executes all of its suites and finds--\_but does not execute\_\_--any hooks and tests therein.
...
8. Mocha runs [global setup fixtures], if any
9. Starting with the "root" suite, Mocha executes:
   1. Any "before all" hooks (for the _root_ suite, this only happens once; see [root hook plugins])
   2. For each test, Mocha executes:
      1. Any "before each" hooks
      2. The test (and reports the result)
      3. Any "after each" hooks
   3. If the current suite has a child suite, repeat the steps in 9. for each child suite; each child suite _inherits_ any "before each" and "after each" hooks defined in its parent
   4. Any "after all" hooks (for the _root_ suite, this only happens once; see [root hook plugins])
10. Mocha prints a summary, if applicable
11. Mocha runs [global teardown fixtures], if any

### Parallel Mode

1. Repeat steps 1 through 6 from [Serial Mode](#serial-mode) above
...
5. Each worker loads `--require`'d modules (but _not_ global fixtures)
6. When a worker receives a test file to run, the worker creates a new Mocha instance _for the single test file_, and:
7. Repeats step 7 from [above](#serial-mode)
8. Repeats step 9 from [above](#serial-mode), with the caveat that the worker _does not_ report test results directly; it holds them in a memory buffer
...
11. Mocha prints a summary, if applicable
